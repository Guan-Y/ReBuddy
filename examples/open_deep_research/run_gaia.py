...
def parse_args():
    ...
    parser.add_argument("--model-id", type=str, default="glm-4.5-air")
...
def get_examples_to_answer(answers_file: str, eval_ds: datasets.Dataset) -> list[dict]:
    ...
    return [line for line in eval_ds.to_list() if line["question"] not in done_questions and not line["file_name"]]
...
def main():
    ...
    # debug
    for example in tqdm(tasks_to_run):
        answer_single_question(example, args.model_id, answers_file, visualizer)


    # # parallel 默认这里我把并发关掉了，以免消耗太多tokens；后续需要测试性能为了效率可以开
